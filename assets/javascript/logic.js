...
var firebaseConfig= {
  ...
  //onclick function to push the userinput to firebase
  $("#submit").on("click", function(event){
    event.preventDefault();
  
    //variable to user input value
    ...
    database.ref().limitToLast(3).on("child_added",
     function(snapshot){
        ...
        var newSite = $("<option id='recent'>").text(snapshot.val().recentlySearched);
     ...
     recentOption.addEventListener("change", dropDown);

     //function to run when the dropdown is changed.
     function dropDown(event) {
            let address = document.getElementById('recent').value;
        getQueryURL(address);
